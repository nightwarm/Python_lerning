...
headers = {
    'Host': host,
    'Referer': 'https://m.weibo.cn/u/1663072851',
...
def parse_page(json):
    ...
    videoIteams = json.get('data').get('cards').get('mblog').get('page_info').get('media_info')
    for videoIteam in videoIteams:
        videoIteam = videoIteam.get('media_info')
        if videoIteam:
            videoData = {
                'videoWatch':videoIteam.get('online_users_number')
            }
            yield data + videoIteam
